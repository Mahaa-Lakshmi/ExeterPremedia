...
class Convert_English_to_French:
    ...
    def readDictionaryAndSet(self):
        ...
        #print("Time to process: in readDictionaryfunction=", time.time() - start_time, "seconds")

    def checkCases(self, key, value):
        # print("Inside Case",key,value)
        if (key.islower()):
            return value.lower()
        elif (key.isupper()):
            return value.upper()
            ...
            for line in f:
                ...
                for words in line.split():
                    if str("".join(re.findall("[\w]+", words.lower()))) in Convert_English_to_French.findword:
                        # print("inside if",str("".join(re.findall("[\w]+", words.lower()))))
                        ...
                        #self.df.append({'Eng': [words], 'French': [frenchword],
                                      #  'Frequency': []}, ignore_index=True)

                        Convert_English_to_French.list3[0].append(str("".join(re.findall("[a-zA-Z]+", words))))
                        Convert_English_to_French.list3[1].append(frenchword)

                        Convert_English_to_French.list3[2].append(0)
                        #self.translatedFile.write(frenchword)

                    else:
                        list2.append(words)
                        #self.translatedFile.write(words)
